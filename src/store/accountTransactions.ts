...
export default defineStore("accountTransactions", () => {
    ...
    try {
      ...
      const fetchMore = async () => {
        ...
        const dataPromise = $fetch(
          `${network.apiDomain}/accounts/${searchValues.value.address}/transactions?${new URLSearchParams(params)}`
        );
  ...
  function download() {
    downloadData(
      ...
      `Transactions_${searchValues.value.address}_${format(new Date(), "HH-mm_MM-dd-yyyy")}`
    );
  }
